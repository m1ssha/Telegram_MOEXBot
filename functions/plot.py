...
from API.moex import get_moex_stock_history, get_moex_stock_history_today, get_imoex
...
def plot_moex_index(days=30):
    ...
    if df is not None and not df.empty:
        ...
        plt.plot(df.index, df["close"], linestyle="-", color="#00c8ff", label="IMOEX")
        ...
        plt.xlabel("Дата", fontsize=12, fontweight="bold", color="white")
        plt.ylabel("Индекс MOEX", fontsize=12, fontweight="bold", color="white")
        plt.title(f"Динамика индекса MOEX за {days} дней", fontsize=14, fontweight="bold", color="white")
        ...
        photo = BufferedInputFile(img_bytes.getvalue(), filename=f"imoex.png")
        return photo
    else:
        return None